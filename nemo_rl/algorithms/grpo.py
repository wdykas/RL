...
def setup(
    ...
    if backend == "megatron":
        if enable_nemo_gym and colocated_inference:
            print(
            ...
            def init_megatron_stack():
                ...
                pg, gen_t = init_megatron_generation(p)
    ...
    if backend == "megatron":
        ...
        if enable_nemo_gym and not colocated_inference:
            # A non-colocated skip-load engine only has final weights and server
            # URLs after the initial refit above. Starting NeMo Gym earlier would
            # publish an empty backend list and initialize MXFP8 CUDA graphs against
            # placeholder tensors.
            nemo_gym_actor, nemo_gym_time = _spinup_nemo_gym(
                policy_generation.dp_openai_server_base_urls,
                generation_config["model_name"],
